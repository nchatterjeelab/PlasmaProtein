...
bimid <- dir("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/window2M_pre/byseq")
bimid <- bimid[str_detect(bimid,".bim")]

res <- character()
res1 <- character()
for (i in 1:length(bimid)){
  tmp <- read.table(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/window2M_pre/byseq/", bimid[i]),
...
df <- unique(df)
write_tsv(df, "/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/PWAS/SNPconvert/2M_ARIC_GRCh38_ID.txt")
writeLines(unique(res), "/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/PWAS/SNPconvert/2M_ARIC_GRCh38_cleaned.txt")
