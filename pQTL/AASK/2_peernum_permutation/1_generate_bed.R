...
dir.create(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/AASK/peernum_permutation/",n_peer,"/bed_file"))
...
for (i in 1:length(seqid)){
  ...
  tmp <- read.table(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/AASK/peernum/invrankpheno/", n_peer, "/", gene, ".pheno"))
  if(i==1){
    tmp <- t(tmp)[-1,]
...
rownames(res1) <- NULL
write_tsv(res1, paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/AASK/peernum_permutation/",n_peer,"/bed_file/allchr.bed"))

for(i in 1:22){
  tmp <- res1[res1[["#Chr"]] == i,]
  tmp$start <- as.integer(tmp$start)
  tmp <- tmp[order(tmp$start,decreasing=F),]
  write_tsv(tmp, paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/AASK/peernum_permutation/",n_peer,"/bed_file/chr",i,".bed"))
  print(i)
}
