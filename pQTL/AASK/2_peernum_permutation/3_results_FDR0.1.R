...
for (i in 1:22){
  tmp <- read.table(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/AASK/peernum_permutation_FDR0.1/",n_peer,"/conditional/chr",i,"/conditional.txt"))
  tmp1 <- tmp[tmp$V19==1,]
  if(i==1){
    res <- tmp1
  }else{
...
write_tsv(res, paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/AASK/peernum_permutation_FDR0.1/",n_peer,"/conditional/allsig.txt"), col_names = F)
