...
a <- dir(paste0("/dcs04/nilanjan/data/jzhang2/pwas/pipeline/AASK/geno/window1M/byseq/"))
a <- a[stringr::str_detect(a,".bed")]
seqid <- seqid[paste0(seqid,".bed") %in% a]

tab1 <- read_tsv("/dcs04/nilanjan/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/pQTL/Tables/1_pQTL_summary_cleaned_2.0.txt")
...
for (i in 1:length(seqid)){
  ...
  geno <- read_plink(paste0("/dcs04/nilanjan/data/jzhang2/pwas/pipeline/AASK/geno/window1M/byseq/",seqid[i]), impute="avg")
  ...
  if(topSNP[i] %in% geno$bim$V2){
    ...
    m <- is.na(snprsid)
    snprsid <- snprsid[!m]
    writeLines(snprsid, paste0("/dcs04/nilanjan/data/jzhang2/pwas/pipeline/AASK/replication/tmp/",seqid[i],".snp"))
    ...
    system(paste0("/dcs04/nilanjan/data/jzhang2/TOOLS/plink/plink2",
                  ...
                  " --extract /dcs04/nilanjan/data/jzhang2/pwas/pipeline/AASK/replication/tmp/",seqid[i],".snp",
                  " --make-bed",
                  " --out /dcs04/nilanjan/data/jzhang2/pwas/pipeline/AASK/replication/tmp/",seqid[i]),
           ignore.stdout = TRUE)

    system(paste0("/dcs04/nilanjan/data/jzhang2/TOOLS/plink/plink1/plink",
                  " --bfile /dcs04/nilanjan/data/jzhang2/pwas/pipeline/AASK/replication/tmp/",seqid[i],
                  " --keep-allele-order",
                  " --r bin4 --threads 1",
                  " --out /dcs04/nilanjan/data/jzhang2/pwas/pipeline/AASK/replication/tmp/",seqid[i]),
           ignore.stdout = TRUE)

    snp_plink <- read.table(paste0("/dcs04/nilanjan/data/jzhang2/pwas/pipeline/AASK/replication/tmp/",seqid[i],".bim"),stringsAsFactors = F)$V2

    LD <- readBin(paste0("/dcs04/nilanjan/data/jzhang2/pwas/pipeline/AASK/replication/tmp/",seqid[i],".ld.bin"),
  ...
  x <- geno$bed[,aasksnp]
  pheno <- read.table(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/AASK/peernum/invrankpheno/40/", seqid[i], ".pheno"))
...
save(seqid, std, topSNP, AASK_SNP, R2_withtopSNP, Estimate, Std.Error, P.value,
     file=paste0("/dcs04/nilanjan/data/jzhang2/pwas/pipeline/AASK/replication/results.RData"))
...
write_tsv(res, paste0("/dcs04/nilanjan/data/jzhang2/pwas/pipeline/AASK/replication/results.txt"))
