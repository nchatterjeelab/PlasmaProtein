...
for (i in 1:22){
  tmp <- read.table(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/White/pQTL/peernum_permutation/",n_peer,"/backup0.01/conditional/chr",i,"/conditional.txt"))
  tmp1 <- tmp[tmp$V19==1,]
  if(i==1){
    res <- tmp1
  }else{
...
write_tsv(res, paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/White/pQTL/peernum_permutation/",n_peer,"/backup0.01/conditional/allsig.txt"), col_names = F)

res <- read.table(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/White/pQTL/peernum_permutation/",n_peer,"/backup0.01/conditional/allsig.txt"), stringsAsFactors = F)
length(unique(res$V1)) # 1785
a <- unique(res$V1)

original0.05 <- read_tsv("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/White/pQTL/Tables/1_pQTL_summary_cleaned.txt")
original0.05 <- original0.05[,c(1,2,3,4,5,6,9,12)]
backup0.01 <- cbind(original0.05, backup0.01=original0.05$SOMAmer %in% a)
write_tsv(backup0.01, paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/White/pQTL/peernum_permutation/",n_peer,"/backup0.01/conditional/backup0.01.txt"))
...
for (i in 1:22){
  tmp <- read.table(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/pQTL/peernum_permutation/",n_peer,"/backup0.01/conditional/chr",i,"/conditional.txt"))
  tmp1 <- tmp[tmp$V19==1,]
  if(i==1){
    res <- tmp1
  }else{
...
write_tsv(res, paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/pQTL/peernum_permutation/",n_peer,"/backup0.01/conditional/allsig.txt"), col_names = F)

res <- read.table(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/pQTL/peernum_permutation/",n_peer,"/backup0.01/conditional/allsig.txt"), stringsAsFactors = F)
length(unique(res$V1)) # 1397
a <- unique(res$V1)

original0.05 <- read_tsv("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/pQTL/Tables/1_pQTL_summary_cleaned.txt")
original0.05 <- original0.05[,c(1,2,3,4,5,6,9,12)]
backup0.01 <- cbind(original0.05, backup0.01=original0.05$SOMAmer %in% a)
write_tsv(backup0.01, paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/pQTL/peernum_permutation/",n_peer,"/backup0.01/conditional/backup0.01.txt"))
