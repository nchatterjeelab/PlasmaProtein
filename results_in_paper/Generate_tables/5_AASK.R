...
res <- read_tsv("/dcl01/chatterj/data/jzhang2/pwas/pipeline/AASK/replication/results.txt")
dim(res)
...
mean(res$fdr < 0.05) # 0.6969482
mean(res$fdr < 0.01) # 0.6082328
res3$fdr <- NA
...
mean(sign(tab12$Beta)==sign(res$beta)) # 0.9178571
cor(tab12$Beta,res$beta) # 0.9279521
## replicated ones
cor(tab12$Beta[res$fdr<0.05],res$beta[res$fdr<0.05]) # 0.9678992
mean(sign(tab12$Beta[res$fdr<0.05])==sign(res$beta[res$fdr<0.05])) # 0.9958974
...
write_tsv(tab4,"/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/pQTL/Tables/4_replicated_AASK_1.0.txt")


library(readr)

dat <- read_tsv("/Users/jnz/Document/JHU/Research/PWAS/Analysis/*Tables/4_replicated_AASK_1.0.txt")

dat$sign1=ifelse(sign(dat$Beta)==1, "+","-")
dat$sign2=ifelse(sign(dat$beta)==1, "+","-")
dat$samedir <- paste0(dat$sign1,"/",dat$sign2)
write_tsv(dat, "/Users/jnz/Document/JHU/Research/PWAS/Analysis/*Tables/4_replicated_AASK_2.0.txt")

dat <- dat[1:1400,]
mean(dat$sign1==dat$sign2)
