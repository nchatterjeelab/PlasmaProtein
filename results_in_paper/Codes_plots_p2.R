...
as3 = read.table("2a.txt",header = F)
...
as4 = read.table("2b.txt",header = F)
...
ss.ea <- read.table("~/2c.txt",header = T)
ss.aa.2 <- read.table("~/2e.txt",header = T)

ss.ea.1 <- read.table("~/2d.txt",header = T)
ss.aa.1 <- read.table("~/2f.txt",header = T)
...
mh2 <- ggplot(ss.aa.2[1:1000,],aes(x=pos,y=-log10(p))) + geom_point(alpha = 1,color = ss.aa.2$cols[1:1000],size = 1.2) +
  ...
  annotate(x=ss.aa.1$pos[100], y=100, geom = "text",label="AA",col = "black",size = 6)
...
mh3 <- ggplot(ss.ea.1[1000:1,],aes(x=pos,y=(pip))) + geom_point(alpha = 1,color = ss.ea.1$cols[1000:1],size = 1.2) +
  ...
  annotate(x=ss.ea.1$pos[100], y=0.9, geom = "text",label="EA",col = "black",size = 6)
# mh3 <- mh3+My_Theme
...
mh4 <- ggplot(ss.aa.1[2:1000,],aes(x=pos,y=(pip))) + geom_point(alpha = 1,color = ss.aa.1$cols[2:1000],size = 1.2) +
  ...
  annotate(x=ss.aa.1$pos[100], y=0.8, geom = "text",label="AA",col = "black",size = 6)
