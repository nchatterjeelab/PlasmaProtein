...
library(stringr)
tmp <- dir("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/White/PWAS/para1/invrank/coefs")
...
library(stringr)
tmp <- dir("/dcl01/chatterj/data/jzhang2/pwas/pipeline/codes/GRCh38/PWAS/White/1_invrank/1_training/submitjobs_rerun_2")
tmp <- tmp[str_detect(tmp, "e[0-9]")]
a <- character()
for (i in 1:length(tmp)){
  b <- readLines(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/codes/GRCh38/PWAS/White/1_invrank/1_training/submitjobs_rerun_2/",tmp[i]))
  b <- paste(b,collapse="; ")
  if(length(b)==0)
    a[i] <- "NA"
  else
