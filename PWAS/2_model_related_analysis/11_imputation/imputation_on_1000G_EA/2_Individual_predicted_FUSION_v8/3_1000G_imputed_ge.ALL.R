...
annota.t <- read_tsv(paste0('/dcs04/nilanjan/data/jzhang2/TWAS/fusion_twas-master/WEIGHTS_v8.ALL/GTEXv8.',tissue,'.pos'))
...
for (i in 1:allgene) {
  ...
  load(paste0("/dcs04/nilanjan/data/jzhang2/TWAS/fusion_twas-master/WEIGHTS_v8.ALL/", WGTi))
  ...
  if(sum(m)==0){
    ...
    snps = snps[m,,drop=F]
    writeLines(snps$V2,
               paste0("/fastscratch/myscratch/jzhang2/PWAS/1000G_imputed_FUSION_v8.ALL/tmp/",tissue,"-",i,".txt"))

    system(paste0("/dcs04/nilanjan/data/jzhang2/TOOLS/plink/plink2",
                  " --bfile /fastscratch/myscratch/jzhang2/1000G/GRCh38/EUR/chr",chr,
                  " --extract /fastscratch/myscratch/jzhang2/PWAS/1000G_imputed_FUSION_v8.ALL/tmp/",tissue,"-",i,".txt",
                  " --make-bed",
                  " --out /fastscratch/myscratch/jzhang2/PWAS/1000G_imputed_FUSION_v8.ALL/tmp/",tissue,"-",i),
           ignore.stdout = TRUE)

    # load in genotype files by chromosome, restrict to matching SNPs and combine
    tmp <- tryCatch({genos = read_plink(paste0("/fastscratch/myscratch/jzhang2/PWAS/1000G_imputed_FUSION_v8.ALL/tmp/",tissue,"-",i),impute="avg") }, error=function(e) e, warning=function(w) w)
    ...
    if(length(a)>0){
      ...
      if(nrow(genos$bim)==0){
        pred_matrix[,i] = 0
        system(paste0("rm -rf /fastscratch/myscratch/jzhang2/PWAS/1000G_imputed_FUSION_v8.ALL/tmp/",tissue,"-",i,".*"))
        next
      }
    }else{
...
write_tsv(as.data.frame(pred_matrix), paste0("/dcs04/nilanjan/data/jzhang2/pwas/pipeline/Results_GRCh38/White/PWAS/1000G_imputed/1000G_imputed_FUSION_v8.ALL/",tissue,".txt"))
