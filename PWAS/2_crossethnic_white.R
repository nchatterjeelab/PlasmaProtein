...
annota.pwas <- read_tsv("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/PWAS/para1/invrank/Plasma_Protein.pos")
...
for (i in 1:length(seq)){
  ...
  load(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/White/PWAS/para1/invrank/coefs_remove_ambiguous_snp/", gene, ".wgt.RDat"))

  if(sum(wgt.matrix[,"enet"]!=0)==0){
    load(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/PWAS/para1/invrank/coefs_remove_ambiguous_snp/", gene, ".wgt.RDat"))
    hsq.all[i] <- hsq[1]
    rsq.cross[i] <- 0
  ...
  a <- read_plink(paste0('/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/window1M/byseq_remove_ambiguous_snp/', gene))
  b <- t(a$X)
  ...
  if(length(snpid)==0){
    ...
    tmp <- read.table(paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/Black/pQTL/peernum/invrankpheno/70/",gene,".pheno"))
...
dir.create("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/CrossEthnic")
write_tsv(res, paste0("/dcl01/chatterj/data/jzhang2/pwas/pipeline/Results_GRCh38/CrossEthnic/EAtoAA_Predicted_Accuracy.txt"))
