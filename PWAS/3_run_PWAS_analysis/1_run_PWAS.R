...
## clean supp table
library(stringr)
dat <- read_tsv(paste0("/Users/jnz/Document/JHU/Research/PWAS/Analysis/500Kb/*RData/PWAS_Urate_CI.txt"))
tmp <- str_split(dat$FILE,"/|[.]")
dat$FILE <- unlist(lapply(tmp, function(x){x[length(x)-2]}))
dat <- dat[,-1]
dat <- dat[,-which(colnames(dat)=="NWGT")]
m <- dat$PWAS.P< 3.7*10^(-5)
m[is.na(m)] <- F
results <- rbind(dat[m,], dat[-m,])
write_tsv(results, paste0("/Users/jnz/Document/JHU/Research/PWAS/Analysis/500Kb/*Tables/Urate_full_suppl.txt"))

dat <- read_tsv(paste0("/Users/jnz/Document/JHU/Research/PWAS/Analysis/500Kb/*RData/PWAS_Gout_CI.txt"))
tmp <- str_split(dat$FILE,"/|[.]")
dat$FILE <- unlist(lapply(tmp, function(x){x[length(x)-2]}))
dat <- dat[,-1]
dat <- dat[,-which(colnames(dat)=="NWGT")]
...
m[is.na(m)] <- F
results <- rbind(dat[m,], dat[-m,])
write_tsv(results, paste0("/Users/jnz/Document/JHU/Research/PWAS/Analysis/500Kb/*Tables/Gout_full_suppl.txt"))
